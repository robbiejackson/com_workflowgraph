/**
 * @class draw2d.policy.connection.WorkflowDragConnectionCreatePolicy
 *
 * This is the same as the draw2d.policy.connection.WorkflowDragConnectionCreatePolicy except that in the 
 * createConnection it passes the request source and target nodes so that the function can determine the 
 * source and target ports, and can return different connections depending on them.
 * NOTE THAT THE SOURCE AND TARGET IN THE DRAW2D CODE ARE THE WRONG WAY ROUND
 *
 * Subsequent comments are draw2d comments
 *
 ...
 */
draw2d.policy.connection.WorkflowDragConnectionCreatePolicy = draw2d.policy.connection.ConnectionCreatePolicy.extend({

    NAME : "draw2d.policy.connection.WorkflowDragConnectionCreatePolicy",
        ...
        if (this.mouseDraggingElement !== null) {
            ...
            if (this.currentDropTarget !== null) {
                ...
                if(this.currentDropTarget instanceof draw2d.Port){
                    ...
                    if(command!==null){
						// NEXT LINE HAS BEEN CHANGED FOR JOOMLA WORKFLOW
                        command.setConnection(this.createConnection(request.target, request.source));
